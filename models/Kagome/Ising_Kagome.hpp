...
private:
	static const int N_SL_KgLatt = 3;
	static const int z_common = 4;
	static const int z_common_half = z_common / 2;
	static constexpr int z_KgLatt[N_SL_KgLatt] = { z_common ,z_common, z_common};
public:
	KagomeLattice(const std::vector<int>& L_spec)
	: Lattice(N_SL_KgLatt, z_KgLatt, L_spec) {
		setup_NN_network();
	};
	...
	double eval_energy() const {
		...
		for (int site_idx = 0; site_idx < lattice._N_sites(); site_idx++) {
			for (int bond_index = 0; bond_index < z; bond_index++) {
				...
				energy_tmp +=0.5*J* spin[site_idx]._sz() * spin[j]._sz();
			}
		}
