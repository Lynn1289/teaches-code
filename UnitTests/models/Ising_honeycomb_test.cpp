...
TEST_CASE("HoneycombLatticeIsingSystem 2") {
	const int dim = 2;
	...
	REQUIRE(test_system.eval_energy() == -15.0);
	REQUIRE(test_system.eval_mz() == 14.0);
}

TEST_CASE("HoneycombLatticeIsingSystem Integer Representation of States (1)") {
	const int dim = 2;
	std::vector<int> L(dim);
	L[0] = 3;
	L[1] = 3;
	honeycombLatticeIsingSystem test_system(L);
	REQUIRE(test_system._maxrep_state() == 262143);
	
	test_system.set_state_by_code(0);
	REQUIRE(test_system.eval_mz() == -18.0);
	REQUIRE(test_system.eval_energy() == -27.0);
	
	test_system.set_state_by_code(262143);
	REQUIRE(test_system.eval_mz() == 18.0);
	REQUIRE(test_system.eval_energy() == -27.0);
}

TEST_CASE("honeycombLatticeIsingSystem Observables Brute-Force Counting") {
	const int dim = 2;
	std::vector<int> L(dim);
	L[0] = 2;
	L[1] = 2;
	const int n_sites = honeycombLatticeIsingSystem::eval_n_spins(L);
	const std::vector<double> beta{ 0, 0.3, 1.1, 3.1};
	...
	for (int i = 160; i < 208; i++) {
		E[i] = 2;
	}
	for (int i = 208; i < 238; i++) {
		E[i] = 4;
	}
	...
	double mz_sq;double ene;double ene_sq;double z;double ene_norm;double ene_sq_norm;
	...
	for ( unsigned int beta_idx = 0; beta_idx < beta.size(); beta_idx++ ) {
		...
		mz_sq=0; ene=0; ene_sq=0;z=0;
		for( unsigned int E_idx = 0; E_idx < 256; E_idx++ ){
		//const double mz_sq = 16.0 * (1.0 + std::exp(-8.0 * b * Jabs)) / (6.0 * std::exp(-8.0 * b * Jabs) + (1.0 + std::exp(-16.0 * b * Jabs)));
		ene = ene-E[E_idx]*std::exp(E[E_idx] * b * Jabs);
		ene_sq=ene_sq+E[E_idx]*E[E_idx]*std::exp(E[E_idx] * b * Jabs);
		z = z+std::exp(E[E_idx] * b * Jabs);
		}
		mz_sq=512+64.0 * std::exp(-6.0 * b * Jabs)+192.0 * std::exp(-2.0 * b * Jabs)+192.0 * std::exp(2.0 * b * Jabs)+384.0 * std::exp(4.0 * b * Jabs)+576.0 * std::exp(6.0 * b * Jabs)+128.0 * std::exp(12.0 * b * Jabs);
		ene_norm=ene/z;
		ene_sq_norm=ene_sq/z;
		mz_sq=mz_sq/z;
		...
		REQUIRE_THAT(data_bundle._get_exact_energy(beta_idx), Catch::Matchers::WithinRel(ene_norm, 1e-12));
		REQUIRE_THAT(data_bundle._get_exact_C(beta_idx), Catch::Matchers::WithinRel(b * b * (ene_sq_norm - ene_norm * ene_norm), 1e-6));
	}
}
