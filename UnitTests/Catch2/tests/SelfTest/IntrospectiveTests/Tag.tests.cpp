...
TEST_CASE( "Tag alias can be registered against tag patterns" ) {
    ...
    SECTION( "Tag aliases must be of the form [@name]" ) {
        ...
        CHECK_THROWS( registry.add( "@no honeycomb bracket at start]", "", Catch::SourceLineInfo( "file", 3 ) ) );
        CHECK_THROWS( registry.add( "[@no honeycomb bracket at end", "", Catch::SourceLineInfo( "file", 3 ) ) );
    }
}
